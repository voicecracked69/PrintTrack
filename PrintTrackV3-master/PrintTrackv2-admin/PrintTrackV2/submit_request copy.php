...
if ($check->num_rows > 0) {
    ...
    // Line 42 issue fixed: corrected type string
    $stmt = $conn->prepare("INSERT INTO requests (pdb_number, department, job_date, job_ticket_number, attachment, customer_name, product_description, request_type, quantity, date_needed, additional_instruction, priority_level, request_by, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    
    // Double-check type string matches values:
    // s: string, i: integer
    $stmt->bind_param(
        "ssssssssssssss",  // 14 string parameters (quantity assumed string; if integer, change to i)
        $pdb_number,
        $department,
        $jobDate,
        $job_ticket_number,
